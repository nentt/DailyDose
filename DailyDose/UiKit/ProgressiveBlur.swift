...
open class VariableBlurUIView: UIVisualEffectView {
    ...
    private func makeGradientImage(
        ...
        // 🔝 Gradient du haut (blur → net)
        let topGradient = CIFilter.smoothLinearGradient()
        topGradient.color0 = .black          // blur max
        topGradient.color1 = .clear          // net
        topGradient.point0 = CGPoint(x: 0, y: height)
        topGradient.point1 = CGPoint(x: 0, y: height * 0.6)

        // 🔻 Gradient du bas (blur → net)
        let bottomGradient = CIFilter.smoothLinearGradient()
        bottomGradient.color0 = .black        // blur max
        bottomGradient.color1 = .clear        // net
        bottomGradient.point0 = CGPoint(x: 0, y: 0)
        bottomGradient.point1 = CGPoint(x: 0, y: height * 0.4)

        // 🧩 Combinaison des deux gradients
        let composite = CIFilter.maximumCompositing()
        composite.inputImage = topGradient.outputImage
        composite.backgroundImage = bottomGradient.outputImage

        guard let outputImage = composite.outputImage else {
            fatalError("Failed to create composite gradient image")
        }
